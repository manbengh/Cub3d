...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/16 14:11:44 by ahbey             #+#    #+#             */
/*   Updated: 2025/04/16 16:06:02 by ahbey            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calculate_step_dist(t_mlx *my_mlx, t_ray *ray)
{
	...
	{
		ray->step_x = 1;
		ray->side_dist.x = (ray->map_x + 1.0 - my_mlx->pos.x)
			* ray->delta_dist.x;
	}
	if (ray->ray_dir.y < 0)
	{
	...
	{
		ray->step_y = 1;
		ray->side_dist.y = (ray->map_y + 1.0 - my_mlx->pos.y)
			* ray->delta_dist.y;
	}
}
...
void	perform_dda(t_cub *cub, t_ray *ray)
{
	...
	if (ray->side == 0)
	{
		ray->perp_wall_dist = (ray->map_x - cub->my_mlx->pos.x + (1
					- ray->step_x) / 2) / ray->ray_dir.x;
		cos_angle = fabs(ray->ray_dir.x * cub->my_mlx->dir.x + ray->ray_dir.y
				* cub->my_mlx->dir.y);
	}
	else
	{
		ray->perp_wall_dist = (ray->map_y - cub->my_mlx->pos.y + (1
